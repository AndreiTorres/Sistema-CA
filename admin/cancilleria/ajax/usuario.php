		...
		switch ($_SESSION['tipousuario']){
			...
			case 'Administrador Cancillería':
				while ($reg=$rspta->fetch_object()) {
					$data[]=array(
						"0"=>"",
						"1"=>$reg->idtipousuario=="Administrador"?"":'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idusuario.')"><i class="fa fa-pencil"></i></button>'.' '.'<button class="btn btn-info btn-xs" onclick="mostrar_clave('.$reg->idusuario.')"><i class="fa fa-key"></i></button>'.' '.'<button class="btn btn-primary btn-xs" onclick="desactivar('."'".$reg->codigo_persona."'".')"><i class="fa fa-close"></i></button>',
						"2"=>$reg->apellidos.' '.$reg->nombre,
						"3"=>$reg->idtipousuario,
						"4"=>"<img src='../../files/usuarios/".$reg->imagen."' height='50px' width='50px'>",
						"5"=>$reg->departamento
